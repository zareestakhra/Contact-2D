function [fw_unsorted] = create_fw_timestep_ramp( KE, ...
    load_distribution_x, load_distribution_y, timestep_number, Number_of_timesteps, aaa, Number_of_Nodes )
...
T=linspace(0.0001,aaa*1000, Number_of_timesteps);

load_distribution=T(1,timestep_number)*load_distribution;

ramp=linspace(0, 1, Number_of_Nodes);

for iu=1:length(ramp)
    load_distribution(2*iu-1,1)=load_distribution(2*iu-1,1)*ramp(1,iu);
    load_distribution(2*iu,1)=load_distribution(2*iu,1)*ramp(1,iu);
end
