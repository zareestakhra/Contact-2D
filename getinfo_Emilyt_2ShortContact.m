function [Anodes_contact_bodyT, KCT, KET]=getinfo_Emilyt_2ShortContact()
...
coardinates=nodes(Anodes_ext_loadedT, 1:4);
Cor=sortrows(coardinates,2);
Anodes_ext_loadedT=Cor(:,1)';
...
for j = 1:length(Anodes_internalT)
        Mnodes_internal(1, 2*(j-1)+1) = 6.*(Anodes_internalT(1,j)-1)+1;
        Mnodes_internal(1, 2*(j-1)+2) = 6.*(Anodes_internalT(1,j)-1)+2;
end
...
%KCTemp=KCT(SortedNodeNumber_DOF,SortedNodeNumber_DOF);
%KC=KCTemp;

%KETTemp=KET(SortedNodeNumber_DOF,:);
%KET=KETTemp;

save('KCEmilyT_Short_Contact.mat');
